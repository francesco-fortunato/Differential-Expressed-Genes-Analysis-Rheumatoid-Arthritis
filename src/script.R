...
###########################################
series = "GSE93272" #https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE93272
tmp <- getGEO(GEO = series) #show(tmp)
set <- tmp$GSE93272_series_matrix.txt.gz
pData <- phenoData(set)
metadata <- pData@data
aData <- assayData(set)
...
list <- split(metadata$geo_accession,
              metadata$`disease state:ch1`)
# We have to take healthy control and RA
control <- list$`healthy control`
case <- list$RA # for retrieving instead MCI data (case <- list$MCI)
